Shader "Custom/URP/Dither_v3Shader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Sony PS1 4x4 ordered dither matrix.
                ...
                // Remap from [-4,+3] into roughly [-0.5,+0.5].
                return ps1[iy * 4 + ix] / 8.0;
            }

            float GetBayerThreshold(int x, int y, float size)
            {
                if (size < 3.0) return Bayer2x2(x, y);
                if (size < 6.0) return Bayer4x4(x, y);
                if (size < 12.0) return Bayer8x8(x, y);
                return Bayer16x16(x, y);
            }
            ...
            float ApplyPerceptualCurve(float luminance, float gammaValue)
            {
                return pow(saturate(luminance), gammaValue);
            }

            float RemovePerceptualCurve(float brightness, float gammaValue)
            {
                return pow(saturate(brightness), 1.0 / max(gammaValue, 0.0001));
            }
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                {
                    signedThreshold = PS1Matrix4x4((int)pixelPos.x, (int)pixelPos.y);
                }
                else
                {
                    float threshold = GetBayerThreshold((int)pixelPos.x, (int)pixelPos.y, _BayerSize);

                    // Convert Bayer threshold from [0,1] into [-0.5,0.5]
                    signedThreshold = threshold - 0.5;
                }

                float luminance = GetLinearLuminance(col.rgb);

                float brightness;
                float ditheredBrightness;
                float targetLuminance;

                if (_UsePerceivedBrightness > 0.5)
                {
                    // Dither in perceptual brightness space.
                    brightness = ApplyPerceptualCurve(luminance, _PerceptualGamma);
                    ditheredBrightness = saturate(brightness + signedThreshold * _DitherStrength);
                    targetLuminance = RemovePerceptualCurve(ditheredBrightness, _PerceptualGamma);
                }
                else
                {
                    // Dither in direct linear brightness space.
                    brightness = luminance;
                    ditheredBrightness = saturate(brightness + signedThreshold * _DitherStrength);
                    targetLuminance = ditheredBrightness;
                }

                // Shift the colour by the luminance difference so hue stays broadly intact.
                float luminanceDelta = targetLuminance - luminance;
                float3 dithered = col.rgb + luminanceDelta;

                return half4(saturate(dithered), col.a);
            }
            ENDHLSL
        }
    }
