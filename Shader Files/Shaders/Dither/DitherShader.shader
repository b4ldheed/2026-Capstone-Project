...
Shader "Custom/URP/DitherShader"
{
    Properties
    {
        _DitherStrength ("Dither Strength", Range(0,1)) = 0.15 // Controls how strongly the Bayer dither pattern will affect the final image.
        _BayerSize ("Bayer Matrix Size", Float) = 16 // Selects which size Bayer matrix to use for the dither.
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                float _DitherStrength; // Brightness offset strength from dither pattern.
                float _BayerSize; // Bayer matrix size.
            CBUFFER_END

            float Bayer2x2(int x, int y)
            {
                // 2x2 Bayer matrix threshold values.
                ...
                // Wrap coords into 2x2 matrix range.
                int ix = x & 1;
                int iy = y & 1;
            ...
            // Chooses which Bayer matrix to use based on the material/script.
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                // Read the current screen colour from the fullscreen pass texture.
                float2 uv = input.texcoord;
                half4 col = SAMPLE_TEXTURE2D_X(_BlitTexture, sampler_LinearClamp, uv);

                // Convert UV coords into pixel coords so dither pattern alligns to the screen.
                float2 pixelPos = uv * _ScreenParams.xy;
                float threshold = GetBayerThreshold((int)pixelPos.x, (int)pixelPos.y, _BayerSize);

                // Convert Bayer threshold from [0,1] to [-0.5,0.5].
                float signedThreshold = threshold - 0.5;

                // Apply ordered dither as a small brightness offset.
                float3 dithered = col.rgb + signedThreshold * _DitherStrength;

                // Clamp result back to valid colour range and return.
